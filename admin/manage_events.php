...
// ==========================================
// Ini logika untuk menangkap tombol Yes atau No dari modal
if (isset($_POST['process_homepage'])) {
    $id = (int) $_POST['id'];
    
    // Ambil value dari tombol yang diklik ('Yes' atau 'No')
...
if (isset($_POST['Simpan'])) {
    ...
    if (move_uploaded_file($tmp, $folder . $new_poster)) {
        // Default homepage saat create adalah 'No'
                            ...
                            <table id="eventTable" class="table table-sm table-bordered table-hover align-middle" style="font-size: 0.9rem;">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th>Link Registration</th>
                                        <th>Status</th>
                                        <th>Homepage</th>
                                        <th>Link page event</th>
                                ...
                                <tbody>
                                    ...
                                    while ($row = mysqli_fetch_assoc($query)) {
                                        $startDate = date("F dS Y", strtotime($row['start_date'])); 
                                        $endDate   = date("F dS Y", strtotime($row['end_date']));
                                    ?>
                                    <tr>
                                        <td><?= $no++ ?></td>
                                        <td><img src="images/events/<?= $row['poster'] ?>" width="70" class="img-thumbnail"></td>
                                        <td><?= htmlspecialchars($row['event_name']) ?></td>
                                        <td><?= $row['event_year'] ?></td>
                                        <td><?= htmlspecialchars(substr($row['event_topic'], 0, 50)) ?>...</td>
                                        <td>Start :<?= $startDate ?><br>End :<?= $endDate ?></td>
                                        <td><a href="<?= htmlspecialchars($row['link_registration']) ?>" target="_blank">#</a></td>
                                        <td><?= $row['status'] == 1 ? 'Aktif' : 'Non Aktif' ?></td>
                                        <td>
                                            <?php if($row['homepage'] == 'Yes'): ?>
                                                <span class="badge bg-primary">Yes</span>
                                            <?php else: ?>
                                                <span class="badge bg-light text-dark border">No</span>
                                            <?php endif; ?>
                                        </td>
                                        <td><?= htmlspecialchars($row['linkpage_event']) ?></td>
                                        <td>
